...
class MinuteCrawler:
                ...
                UpdateOne(
                    {
                        ...
                        'index': doc['index'], 'time': doc['time'], 'minute': doc['minute']
                    },
                    {'$set': doc},
                    upsert=True)
            )
    ...
    def crawl(self, begin_date=None, end_date=None):
        ...
        for code in codes:
            # 抓取不复权的价格
            df_daily = ts.get_k_data(code, autype=None, ktype='1', start=begin_date, end=end_date)
            self.save_data(code, df_daily, self.minute, {'index': False, 'minute': 1})
